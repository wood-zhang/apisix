...
function _M.http_init(prometheus_enabled_in_stream)
    ...
    metrics.status = prometheus:counter("http_status",
            ...
            {"code", "route", "matched_uri", "matched_host", "service", "consumer",
            unpack(extra_labels("http_status"))})

    metrics.latency = prometheus:histogram("http_latency",
        "HTTP request latency in milliseconds per service in APISIX",
        {"type", "route", "service", "consumer", unpack(extra_labels("http_latency"))},
        DEFAULT_BUCKETS)

    metrics.bandwidth = prometheus:counter("bandwidth",
            "Total bandwidth in bytes consumed per service in APISIX",
            {"type", "route", "service", "consumer", unpack(extra_labels("bandwidth"))})
...
function _M.http_log(conf, ctx)
    ...
    metrics.latency:observe(latency,
        gen_arr("request", route_id, service_id, consumer_name,
    ...
    metrics.bandwidth:inc(vars.request_length,
        gen_arr("ingress", route_id, service_id, consumer_name,
        unpack(bandwidth_extra_label_values)))

    metrics.bandwidth:inc(vars.bytes_sent,
        gen_arr("egress", route_id, service_id, consumer_name,
        unpack(bandwidth_extra_label_values)))
end
